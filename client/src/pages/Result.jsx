...
import { motion } from 'framer-motion'
import { AppContext } from '../context/AppContext'
import VoiceInput from '../components/VoiceInput' // 👈 added
...
const Result = () => {
  ...
  return (
    ...
    >
      ...
      {!isImageLoaded && (
        <div className="flex items-center bg-neutral-500 text-white text-sm px-4 py-2 rounded-full w-full max-w-xl mt-10">
          <input
            type="text"
          ...
          <VoiceInput onTextGenerated={(text) => setInput(text)} />

          <button
            type="submit"
            className="ml-2 bg-zinc-900 px-6 py-2 rounded-full"
          >
            Generate
          </button>
        </div>
